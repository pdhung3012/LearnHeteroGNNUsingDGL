...
fopDistinctValue=fopResult+'uniqueValues/'
createDirIfNotExist(fopResult)
createDirIfNotExist(fopDistinctValue)
...
YList=Y.tolist()
numberScale=10
fpStatInfo=fopResult+'stat.txt'
f1=open(fpStatInfo,'w')
f1.write('minStat\tmaxStat\tmeanStat\tmedianStat\tsizeUniqueValue\n')
f1.close()
fpScaleRange=fopResult+'scale.txt'
f1=open(fpScaleRange,'w')
f1.write('{}\t{}\n'.format('Feature','\t'.join(['{}'.format(i+1) for i in range(0,numberScale)])))
f1.close()
for i in range(0,lenColumns):
    columnItem=X.getcol(i).toarray().tolist()
    columnItem=[it[0] for it in columnItem]
    lstOutputFeatures, setOutputFeatures, lstStatistics, lstScaleByRange=getFeatureStatistics(columnItem,numberScale)
    print('begin {} {}'.format(i + 1, lstHeaderCols[i]))
    f1=open(fpStatInfo,'a')
    f1.write('{}\t{}\n'.format(lstHeaderCols[i],'\t'.join(['{}'.format(it) for it in lstStatistics])))
    f1.close()
    f1=open(fpScaleRange,'a')
    f1.write('{}\t{}\n'.format(lstHeaderCols[i],'\t'.join(['{}'.format(it) for it in lstScaleByRange])))
    f1.close
    f1=open(fopDistinctValue+lstHeaderCols[i]+'.txt','w')
    f1.write('\n'.join(['{}'.format(it) for it in sorted(list(setOutputFeatures))]))
    f1.close()
    print('end {} {}'.format(i+1,lstHeaderCols[i]))

lstOutputFeatures, setOutputFeatures, lstStatistics, lstScaleByRange = getFeatureStatistics(YList, numberScale)
strLabel='z_label'
print('begin {} {}'.format(i + 1, strLabel))
f1 = open(fpStatInfo, 'a')
f1.write('{}\t{}\n'.format(strLabel, '\t'.join(['{}'.format(it) for it in lstStatistics])))
f1.close()
f1 = open(fpScaleRange, 'a')
f1.write('{}\t{}\n'.format(strLabel, '\t'.join(['{}'.format(it) for it in lstScaleByRange])))
f1.close
f1 = open(fopDistinctValue + strLabel + '.txt', 'w')
f1.write('\n'.join(['{}'.format(it) for it in sorted(list(setOutputFeatures))]))
f1.close()
print('end {} {}'.format(i + 1, strLabel))
