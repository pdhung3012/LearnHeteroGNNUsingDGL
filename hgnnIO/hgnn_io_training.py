...
def main(TRAIN=True):
    ...
    if TRAIN:
        ...
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=my_config.optimizer['bs'], shuffle=True, collate_fn=collate_fn, num_workers=5,
            prefetch_factor=5)
    else:
        dataset = get_dataset(my_config.data['test_path'])
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=my_config.optimizer['test_bs'], shuffle=False, collate_fn=collate_fn, num_workers=5,
            prefetch_factor=5)
    t2 = time.time()
